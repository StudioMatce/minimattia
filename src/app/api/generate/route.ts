...
export async function POST(request: Request) {
  try {
    ...
    if (geminiAvailable) {
      try {
        if (isTextMode) {
          log("Trying Gemini SVG from text prompt...");
          const result = await textPromptToSvgGeminiBoth(textPrompt);
          ...
          log("Gemini SVG (text) succeeded");
        } else {
          log("Trying Gemini SVG (two-pass)...");
          const result = await sketchToSvgGeminiBoth(imageBase64);
          svgContentLight = result.light;
          svgContentDark = result.dark;
    ...
    log(`Generate API error: ${message}`);
