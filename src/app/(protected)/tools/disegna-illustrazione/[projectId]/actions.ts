...
export async function generateDiagram(
  ...
  if (geminiAvailable) {
    try {
      ...
      const result = await sketchToIllustrationGeminiBoth(imageBase64, learningContext);
...
async function generateDiagramFromText(projectId: string, textPrompt: string, learningContext?: string) {
  ...
  log("Regenerating illustration from text prompt...");
  const result = await textPromptToIllustrationGeminiBoth(textPrompt, learningContext);
  svgContentLight = result.light;
  svgContentDark = result.dark;
  diagramJson = { directSvg: true, engine: "gemini", hasDarkMode: true, source: "text-prompt" };
  log("Text prompt illustration regeneration succeeded");
