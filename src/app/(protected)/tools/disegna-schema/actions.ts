  ...
  return db
    ...
    .where(
      and(
        ...
        eq(projects.toolType, "disegna-schema")
      )
    )
...
export async function createProject(name: string) {
  ...
  // Link to user's brand config if one exists
  ...
  const [project] = await db
    ...
    .values({
      userId: user.id,
      name,
      toolType: "disegna-schema",
      brandConfigId: configs[0]?.id ?? null,
    })
    .returning();

  redirect(`/tools/disegna-schema/${project.id}`);
}
...
export async function renameProject(projectId: string, newName: string) {
  ...
  revalidatePath("/tools/disegna-schema");
}
